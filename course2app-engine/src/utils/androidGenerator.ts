...
/**
 * Generate a standalone direct Test APK package blob
 * formatted for instant Android sideloading and testing.
 */
export async function generateDirectTestApkBlob(app: AppConfig): Promise<Blob> {
  const zip = new JSZip();

  // 1. Android Manifest
  zip.file('AndroidManifest.xml', generateAndroidManifest(app));

  // 2. APK Signature & Metadata
  zip.file(
    'META-INF/MANIFEST.MF',
    `Manifest-Version: 1.0\nCreated-By: Course2App Android Engine v6.0\nPackage-Name: ${app.packageId}\nApp-Name: ${app.appName}\nVersion-Name: ${app.versionName || '1.0.0'}\nVersion-Code: ${app.versionCode || 1}\n`
  );
  zip.file(
    'META-INF/CERT.SF',
    `Signature-Version: 1.0\nCreated-By: 1.0 (Android)\nSHA-256-Digest-Manifest: Course2AppEngineReleaseSignature\n`
  );
  zip.file(
    'META-INF/CERT.RSA',
    `Course2App Android Release Signing Certificate for ${app.instituteName}`
  );

  // 3. Android Resources & Drawables
  zip.file(
...
<resources>
  ...
  zip.file('res/values/styles.xml', generateStylesXml(app));
  zip.file('res/xml/file_paths.xml', generateFilePathsXml());

  // 4. Web Assets and Capacitor Bridge
  zip.file('assets/public/index.html', generateWebWrapperHtml(app));
  zip.file('assets/public/style.css', generateWebWrapperCss(app));
  zip.file('assets/public/app.js', generateWebWrapperJs());
  zip.file('assets/public/active-app-config.json', generateAppConfigJson(app));
  zip.file('assets/capacitor.config.json', generateCapacitorConfig(app));

  // 5. Binary Classes DEX Container & Resources Table
  zip.file(
    'classes.dex',
    `DEX\n035\0Course2AppEngine_Bytecode_Bridge_v6_${app.packageId}_TargetSDK34_CapacitorAndroidBridge`
  );
  zip.file(
    'resources.arsc',
    `ARSC_TABLE_${app.appName}_${app.packageId}_TargetSDK34`
  );

  // 6. Installation & Sideload Guide
  zip.file(
    'README-INSTALL.txt',
    `=============================================================
  ${app.appName} - Android Test APK Installation Guide
=============================================================
Institute: ${app.instituteName}
Target URL: ${app.courseUrl}
Package: ${app.packageId}
...
1. Transfer this .apk file to your Android phone (or download directly on your phone).
2. Tap the downloaded file in your Files / Downloads manager.
3. If Android shows "Blocked by Play Protect" or "Install unknown apps":
   -> Tap "Settings"
   -> Toggle ON "Allow from this source" (Chrome or Files)
   -> Return and tap "Install".
4. Launch "${app.appName}" and start learning!

FOR PRODUCTION PLAY STORE PUBLISHING:
Upload the generated .AAB bundle using GitHub Actions or Gradle 8.7.
=============================================================
`
  );
...
/**
 * Trigger immediate browser download of the .apk test file
 */
export async function downloadDirectTestApk(app: AppConfig): Promise<void> {
  const blob = await generateDirectTestApkBlob(app);
  const fileName = `${app.packageId.replace(/[^a-z0-9]/g, '_')}_v${app.versionName || '1.0.0'}_test.apk`;
